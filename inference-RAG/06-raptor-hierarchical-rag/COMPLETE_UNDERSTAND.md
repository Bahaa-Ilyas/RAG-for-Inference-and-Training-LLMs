# Complete Understanding: RAPTOR / hierarchical retrieval

## Core idea
RAPTOR recursively clusters chunks and summarizes them into a tree so retrieval can search both granular passages and higher-level summaries. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Standard RAG over long documents can miss broader context, while sending the full document is too expensive. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Hierarchical vector retrieval over leaf chunks and recursively generated summary nodes..
...
## Mathematical view
Let leaves be C. Cluster embeddings into groups G_l at each level l and summarize each group into node s_g. Retrieval maximizes sim(q,node) across levels. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. RAPTOR / hierarchical retrieval changes that by using Hierarchical vector retrieval over leaf chunks and recursively generated summary nodes. and by explicitly addressing: Standard RAG over long documents can miss broader context, while sending the full document is too expensive.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Chunk documents; embed chunks; cluster; summarize clusters; repeat to root; retrieve from multiple levels; expand selected summaries to source chunks.
