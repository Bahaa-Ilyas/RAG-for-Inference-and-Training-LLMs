# Complete Understanding: Reranked RAG

## Core idea
Reranked RAG retrieves a broad candidate set and applies a stronger cross-encoder or LLM reranker before passing compact evidence to the generator. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
First-stage retrievers optimize recall, not final ordering, so irrelevant chunks often occupy the limited context window. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Any first-stage sparse, dense, or hybrid retriever followed by cross-encoder, late-interaction, or LLM reranking..
...
## Mathematical view
A cross-encoder estimates s(q,d)=f_theta([CLS] q [SEP] d). Final ranking sorts candidates by s, optionally mixed with diversity penalties. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Reranked RAG changes that by using Any first-stage sparse, dense, or hybrid retriever followed by cross-encoder, late-interaction, or LLM reranking. and by explicitly addressing: First-stage retrievers optimize recall, not final ordering, so irrelevant chunks often occupy the limited context window.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Retrieve a large candidate set; split or trim candidates for the reranker; score query-document pairs; diversify by source; pass top evidence to the generator.
