# Building Reranked RAG From Scratch
...
Source: [ColBERT and cross-encoder reranking literature](https://arxiv.org/abs/2004.12832). This local SVG is a redraw/synthesis of the important method architecture from the canonical paper or source, not a copied paper figure.
...
# Reranked RAG
        ...
        ## Problem it solves
        First-stage retrievers optimize recall, not final ordering, so irrelevant chunks often occupy the limited context window.

        ## When to use this method
        Use it when answer quality is sensitive to top-k precision, when citations must be accurate, or when the retriever returns many near-duplicates.

        ## When not to use this method
        Avoid it when latency is extremely tight, candidate texts are huge, or your first-stage retrieval already has validated high precision for a simple corpus.

        ## Core components
        First-stage retriever, candidate pool, cross-encoder reranker, diversity filter, context budgeter, generator, citation verifier.

        ## Input and output
        Input is a query and candidate passages. Output is a reranked evidence list with relevance scores and an answer constrained to the selected passages.

        ## PDF use
        Reranking helps PDFs when adjacent chunks have similar text but only one contains the answer, table caption, or required page-specific citation. For text PDFs, preserve page numbers, section headings, captions, reading order, and chunk offsets. For scanned PDFs, run OCR before chunking and store OCR confidence, page image references, and detected tables. Use Unstructured, Docling, Marker, PyMuPDF, pdfplumber, Apache Tika, or Tesseract depending on document quality. Long scientific papers usually need section-aware chunking, parent-child links, equation/caption retention, and table extraction.

        ## Webpage use
        For webpages, reranking should penalize boilerplate, stale copies, and pages whose title or canonical URL does not match the query intent. For webpages, crawl with robots.txt awareness, sitemap support, canonical URL normalization, crawl timestamps, content hashing, boilerplate removal, and duplicate detection. Use Trafilatura, BeautifulSoup, Scrapy, Playwright, readability extraction, and URL canonicalization. Treat retrieved web text as untrusted because prompt injection can be embedded in pages.

        ## Production stack
        BGE rerankers, Cohere Rerank, Jina rerankers, ColBERT, SentenceTransformers CrossEncoder, LlamaIndex postprocessors, LangChain contextual compression.

        ## Minimal example
        Retrieve 80 passages about a product outage, rerank them with a cross-encoder, keep the top 8 unique pages, and ask the LLM to cite only those passages.

        ## Pros
        - Improves precision at low context budgets
- Works with any retriever
- Gives useful relevance scores for monitoring

        ## Cons
        - Adds latency and model cost
- Long chunks may exceed reranker limits
- Can overfit to lexical overlap if the reranker is poorly chosen
