# Complete Understanding: Claim-level verification RAG

## Core idea
Claim-level verification decomposes generated answers into atomic claims and verifies each claim against retrieved evidence before final response. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
A generated answer can be mostly grounded while still containing unsupported details, wrong dates, or over-broad claims. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Post-generation or interleaved retrieval per atomic claim..
...
## Mathematical view
For each claim c_i, estimate support(c_i,E_i), contradiction(c_i,E_i), and citation_precision. Final answer keeps claims above support threshold and below contradiction threshold. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Claim-level verification RAG changes that by using Post-generation or interleaved retrieval per atomic claim. and by explicitly addressing: A generated answer can be mostly grounded while still containing unsupported details, wrong dates, or over-broad claims.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Draft answer; extract atomic claims; retrieve evidence per claim; classify support/contradiction; revise unsupported claims; return cited answer.
