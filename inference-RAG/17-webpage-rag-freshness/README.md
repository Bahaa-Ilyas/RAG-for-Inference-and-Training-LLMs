# Building Webpage RAG with crawler and freshness control From Scratch
...
Source: [Trafilatura documentation](https://trafilatura.readthedocs.io/). This local SVG is a redraw/synthesis of the important method architecture from the canonical paper or source, not a copied paper figure.
...
# Webpage RAG with crawler and freshness control

        ## Summary
        Webpage RAG builds a controlled crawler, cleaned HTML corpus, freshness metadata, and recrawl policy so answers reflect current web content.

        ## Problem it solves
        Naive webpage ingestion duplicates boilerplate, misses dynamic content, ignores canonical URLs, and serves stale pages.

        ## When to use this method
        Use it for documentation sites, blogs, product pages, API references, news-like corpora, and external knowledge bases that change over time.

        ## When not to use this method
        Avoid it when licensing, robots.txt, or terms prohibit crawling, or when the task requires authoritative API data instead of page text.

        ## Core components
        Crawler, robots/sitemap manager, renderer, HTML cleaner, canonicalizer, deduplicator, content hasher, freshness scheduler, retriever, injection filter.

        ## Input and output
        Input is seed URLs and a query. Output is fresh, canonical, deduplicated page evidence with crawl metadata and answer citations.

        ## PDF use
        Linked PDFs should enter the PDF pipeline with the referring URL, crawl timestamp, and file hash preserved as provenance. For text PDFs, preserve page numbers, section headings, captions, reading order, and chunk offsets. For scanned PDFs, run OCR before chunking and store OCR confidence, page image references, and detected tables. Use Unstructured, Docling, Marker, PyMuPDF, pdfplumber, Apache Tika, or Tesseract depending on document quality. Long scientific papers usually need section-aware chunking, parent-child links, equation/caption retention, and table extraction.

        ## Webpage use
        This method directly covers crawling, sitemap ingestion, robots awareness, HTML cleaning, canonical URLs, freshness, versioning, dynamic pages, duplicates, and prompt injection. For webpages, crawl with robots.txt awareness, sitemap support, canonical URL normalization, crawl timestamps, content hashing, boilerplate removal, and duplicate detection. Use Trafilatura, BeautifulSoup, Scrapy, Playwright, readability extraction, and URL canonicalization. Treat retrieved web text as untrusted because prompt injection can be embedded in pages.

        ## Production stack
        Scrapy, Trafilatura, BeautifulSoup, Playwright, sitemap parsers, readability extractors, OpenSearch, Qdrant, content hashing, freshness queues.

        ## Minimal example
        Recrawl API docs weekly, immediately recrawl changed sitemap URLs, deduplicate localized boilerplate, and prefer the newest non-deprecated version in retrieval.

        ## Pros
        - Controls freshness and provenance
- Reduces boilerplate noise
- Works for dynamic documentation

        ## Cons
        - Crawler operations are nontrivial
- Legal and robots constraints matter
- Dynamic rendering is expensive

        ## Related methods
        - [18-version-aware-rag](../18-version-aware-rag/)
...
```bash
python webpage_freshness_rag.py --explain
python examples/run_example.py
```
