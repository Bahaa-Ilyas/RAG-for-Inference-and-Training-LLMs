# Complete Understanding: Contextual compression RAG

## Core idea
Contextual compression filters, extracts, or summarizes retrieved passages so only query-relevant evidence enters the generation context. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Retrievers often return chunks that contain the answer plus unrelated text, exceeding context limits and distracting the generator. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Post-retrieval compression after sparse, dense, hybrid, or hierarchical retrieval..
...
## Mathematical view
Select spans z from document d maximizing relevance(q,z)-lambda*length(z), subject to citation provenance and coverage constraints. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Contextual compression RAG changes that by using Post-retrieval compression after sparse, dense, hybrid, or hierarchical retrieval. and by explicitly addressing: Retrievers often return chunks that contain the answer plus unrelated text, exceeding context limits and distracting the generator.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Retrieve; split into sentences/spans; score spans against query; keep relevant spans under budget; preserve source offsets; generate with citations.
