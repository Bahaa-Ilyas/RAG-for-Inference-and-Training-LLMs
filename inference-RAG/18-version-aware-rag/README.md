# Building Version-aware RAG From Scratch
...
Source: [OpenSearch vector search and filtering documentation](https://opensearch.org/docs/latest/search-plugins/vector-search/). This local SVG is a redraw/synthesis of the important method architecture from the canonical paper or source, not a copied paper figure.
...
# Version-aware RAG
        ...
        ## Problem it solves
        Standard RAG often retrieves obsolete policies or docs, creating answers that are correct for the wrong version.

        ## When to use this method
        Use it for product documentation, legal policies, standards, software APIs, medical guidelines, and any corpus with effective dates.

        ## When not to use this method
        Avoid it when the corpus has no reliable version metadata or when only the latest document is allowed and old content can be purged.

        ## Core components
        Version metadata schema, effective-date parser, supersession graph, diff store, version filters, freshness policy, citation verifier.

        ## Input and output
        Input is a query with explicit or implicit time/version context. Output is evidence and answers scoped to the correct version.

        ## PDF use
        PDF versions need document hash, revision date, effective date, supersedes/superseded-by links, page citations, and section IDs that survive revisions. For text PDFs, preserve page numbers, section headings, captions, reading order, and chunk offsets. For scanned PDFs, run OCR before chunking and store OCR confidence, page image references, and detected tables. Use Unstructured, Docling, Marker, PyMuPDF, pdfplumber, Apache Tika, or Tesseract depending on document quality. Long scientific papers usually need section-aware chunking, parent-child links, equation/caption retention, and table extraction.

        ## Webpage use
        Webpage versions need canonical URL, crawl timestamp, release path, content hash, redirect history, and deprecated-page detection. For webpages, crawl with robots.txt awareness, sitemap support, canonical URL normalization, crawl timestamps, content hashing, boilerplate removal, and duplicate detection. Use Trafilatura, BeautifulSoup, Scrapy, Playwright, readability extraction, and URL canonicalization. Treat retrieved web text as untrusted because prompt injection can be embedded in pages.

        ## Production stack
        OpenSearch, Postgres, Qdrant payload filters, Weaviate filters, temporal metadata stores, diff tools, sitemap recrawl jobs.

        ## Minimal example
        Answer 'How do we configure auth in v2.1?' using v2.1 docs, not current v3.0 docs, and cite the archived URL or PDF version.

        ## Pros
        - Prevents stale or wrong-version answers
- Improves auditability
- Works with compliance workflows

        ## Cons
        - Requires strict metadata governance
- Version inference is hard
- Index size grows with retained history

        ## Related methods
        - [12-metadata-filtered-rag](../12-metadata-filtered-rag/)
- [17-webpage-rag-freshness](../17-webpage-rag-freshness/)
- [17-version-aware-training](../training_rag/17-version-aware-training)
...
```bash
python version_aware_rag.py --explain
python examples/run_example.py
```
