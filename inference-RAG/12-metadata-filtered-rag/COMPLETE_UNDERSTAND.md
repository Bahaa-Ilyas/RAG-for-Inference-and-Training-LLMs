# Complete Understanding: Metadata-filtered RAG / auto-retrieval

## Core idea
Metadata-filtered RAG converts user constraints into structured filters and applies them before or during retrieval. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Semantic similarity alone ignores permissions, dates, departments, versions, document types, products, and jurisdiction constraints. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Vector, sparse, or hybrid retrieval constrained by structured filters..
...
## Mathematical view
Retrieve arg top_k over d where F(d)=true, maximizing sim(q,d). Filter precision and recall are as important as retrieval recall. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Metadata-filtered RAG / auto-retrieval changes that by using Vector, sparse, or hybrid retrieval constrained by structured filters. and by explicitly addressing: Semantic similarity alone ignores permissions, dates, departments, versions, document types, products, and jurisdiction constraints.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Extract constraints; validate against schema; apply filters and ACLs; retrieve; relax safe filters only when recall is insufficient; answer with visible scope.
