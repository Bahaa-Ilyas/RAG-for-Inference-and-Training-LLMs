# Complete Understanding: Tool-augmented RAG / API RAG

## Core idea
Tool-augmented RAG retrieves static context and calls live APIs, databases, calculators, or domain tools when text alone is insufficient. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Documents cannot answer questions requiring current account state, calculations, transactions, or authoritative structured systems. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Static RAG plus dynamic tool calls selected by a planner or deterministic router..
...
## Mathematical view
Planner utility weighs expected information gain of tool call t against cost, latency, permission, and side-effect risk. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Tool-augmented RAG / API RAG changes that by using Static RAG plus dynamic tool calls selected by a planner or deterministic router. and by explicitly addressing: Documents cannot answer questions requiring current account state, calculations, transactions, or authoritative structured systems.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Retrieve policy context; choose required tool; validate arguments; call read-only API first; normalize result; synthesize with source and tool citations.
