# Building Tool-augmented RAG / API RAG From Scratch
...
Source: [Toolformer and ReAct tool-use papers](https://arxiv.org/abs/2302.04761). This local SVG is a redraw/synthesis of the important method architecture from the canonical paper or source, not a copied paper figure.
...
# Tool-augmented RAG / API RAG

        ## Summary
        Tool-augmented RAG retrieves static context and calls live APIs, databases, calculators, or domain tools when text alone is insufficient.

        ## Problem it solves
        Documents cannot answer questions requiring current account state, calculations, transactions, or authoritative structured systems.

        ## When to use this method
        Use it for finance, operations, support, analytics, scheduling, inventory, and systems where answers combine docs with live data.

        ## When not to use this method
        Avoid it when tools lack authorization boundaries, side effects cannot be controlled, or static retrieval is sufficient.

        ## Core components
        Retriever, tool registry, schema validator, planner, API client, permission layer, result normalizer, generator, audit log.

        ## Input and output
        Input is a user task and credentials. Output is an answer grounded in retrieved docs and structured tool results.

        ## PDF use
        PDFs provide procedure, policy, and field definitions; tools provide current values. Cite the PDF for rules and the API result for state. For text PDFs, preserve page numbers, section headings, captions, reading order, and chunk offsets. For scanned PDFs, run OCR before chunking and store OCR confidence, page image references, and detected tables. Use Unstructured, Docling, Marker, PyMuPDF, pdfplumber, Apache Tika, or Tesseract depending on document quality. Long scientific papers usually need section-aware chunking, parent-child links, equation/caption retention, and table extraction.

        ## Webpage use
        Web docs explain API behavior, but live APIs should answer current values; protect against webpage prompt injection that tries to alter tool calls. For webpages, crawl with robots.txt awareness, sitemap support, canonical URL normalization, crawl timestamps, content hashing, boilerplate removal, and duplicate detection. Use Trafilatura, BeautifulSoup, Scrapy, Playwright, readability extraction, and URL canonicalization. Treat retrieved web text as untrusted because prompt injection can be embedded in pages.

        ## Production stack
        LangGraph, OpenAI tool/function calling, LlamaIndex tools, Haystack agents, API gateways, policy engines, structured logging.

        ## Minimal example
        Retrieve refund policy, call order API for delivery date and status, compute eligibility, and return cited policy plus API-derived facts.

        ## Pros
        - Handles dynamic facts and computation
- Reduces hallucinated operational answers
- Supports structured workflows

        ## Cons
        - Security and side-effect risks
- Tool errors affect answers
- Requires strong auditing

        ## Related methods
        - [03-agentic-rag](../03-agentic-rag/)
- [17-webpage-rag-freshness](../17-webpage-rag-freshness/)
- [18-tool-use-rag-training](../training_rag/18-tool-use-rag-training)
...
```bash
python tool_augmented_rag.py --explain
python examples/run_example.py
```
