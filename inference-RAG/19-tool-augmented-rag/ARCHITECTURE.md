# Architecture: Tool-augmented RAG / API RAG

## High-level architecture
Tool-augmented RAG / API RAG uses Static RAG plus dynamic tool calls selected by a planner or deterministic router. to address the specific failure mode: Documents cannot answer questions requiring current account state, calculations, transactions, or authoritative structured systems. The system should expose evidence, scores, metadata, and citations rather than treating generation as a black box.

## Index-time pipeline
Ingest PDFs, webpages, tables, and other files; parse text and layout; split into chunks or structured elements; attach metadata; embed and index into sparse, dense, graph, table, or tool-aware stores as required. For this method, the important implementation point is: Retrieve policy context; choose required tool; validate arguments; call read-only API first; normalize result; synthesize with source and tool citations.
