# Building Table-aware RAG From Scratch
...
Source: [TAPAS: Weakly Supervised Table Parsing via Pre-training](https://arxiv.org/abs/2004.02349). This local SVG is a redraw/synthesis of the important method architecture from the canonical paper or source, not a copied paper figure.
...
# Table-aware RAG
        ...
        ## Problem it solves
        Standard chunking destroys row/column relationships, units, merged cells, captions, and footnotes.

        ## When to use this method
        Use it for financial reports, scientific tables, product matrices, pricing sheets, policy schedules, and PDF tables.

        ## When not to use this method
        Avoid it when tables are decorative or when extraction quality cannot be validated for high-stakes numerical answers.

        ## Core components
        Table detector, table parser, schema normalizer, table store, row/column retriever, SQL/pandas executor, verifier, generator.

        ## Input and output
        Input is a query and table corpus. Output is cited rows, computed values, and an answer with units and table provenance.

        ## PDF use
        PDF table RAG needs layout extraction, header reconstruction, units, captions, footnotes, page numbers, and OCR confidence for scanned tables. For text PDFs, preserve page numbers, section headings, captions, reading order, and chunk offsets. For scanned PDFs, run OCR before chunking and store OCR confidence, page image references, and detected tables. Use Unstructured, Docling, Marker, PyMuPDF, pdfplumber, Apache Tika, or Tesseract depending on document quality. Long scientific papers usually need section-aware chunking, parent-child links, equation/caption retention, and table extraction.

        ## Webpage use
        Web tables should preserve DOM structure, surrounding headings, caption text, URL, crawl timestamp, and repeated header rows. For webpages, crawl with robots.txt awareness, sitemap support, canonical URL normalization, crawl timestamps, content hashing, boilerplate removal, and duplicate detection. Use Trafilatura, BeautifulSoup, Scrapy, Playwright, readability extraction, and URL canonicalization. Treat retrieved web text as untrusted because prompt injection can be embedded in pages.

        ## Production stack
        Docling, Unstructured, pdfplumber, Camelot/Tabula where appropriate, pandas, DuckDB, LlamaIndex table engines, text-to-SQL guardrails.

        ## Minimal example
        Ask 'Which SKU has the highest warranty reserve in 2025?' Retrieve the table, execute a max over the reserve column, and cite row and page.

        ## Pros
        - Preserves numeric structure
- Supports calculations
- Improves citations for rows and columns

        ## Cons
        - Table extraction is brittle
- Merged cells and OCR errors are hard
- Needs validation for computations
...
- [15-table-aware-synthetic-data](../training_rag/15-table-aware-synthetic-data)
...
```bash
python table_aware_rag.py --explain
python examples/run_example.py
```
