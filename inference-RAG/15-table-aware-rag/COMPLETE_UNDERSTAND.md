# Complete Understanding: Table-aware RAG

## Core idea
Table-aware RAG extracts, indexes, retrieves, and reasons over tables as structured data rather than flattening them into ambiguous text. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Standard chunking destroys row/column relationships, units, merged cells, captions, and footnotes. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Hybrid text/table retrieval plus optional structured execution over extracted tables..
...
## Mathematical view
Represent table T as cells c_ij with row and column headers. Retrieval scores combine caption relevance, header match, cell match, and executable query confidence. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Table-aware RAG changes that by using Hybrid text/table retrieval plus optional structured execution over extracted tables. and by explicitly addressing: Standard chunking destroys row/column relationships, units, merged cells, captions, and footnotes.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Detect tables; reconstruct schema; index caption/header/rows; retrieve candidate tables; execute or reason over rows; verify units and cite cells.
