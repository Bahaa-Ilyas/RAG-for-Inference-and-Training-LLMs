# Complete Understanding: Decomposition RAG / sub-question RAG

## Core idea
Decomposition RAG breaks a complex question into sub-questions, retrieves evidence for each, and synthesizes the final answer from the sub-results. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Single retrieval calls fail on multi-hop questions that require comparing entities, time periods, documents, or constraints. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Multiple focused retrieval calls planned from decomposed sub-questions..
...
## Mathematical view
Answer y is conditioned on evidence E_i retrieved for sub-question q_i. Confidence can aggregate min_i support(q_i,E_i) and contradiction penalties. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Decomposition RAG / sub-question RAG changes that by using Multiple focused retrieval calls planned from decomposed sub-questions. and by explicitly addressing: Single retrieval calls fail on multi-hop questions that require comparing entities, time periods, documents, or constraints.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Generate sub-questions; retrieve evidence per sub-question; answer each with citations; synthesize; verify each final claim against sub-evidence.
