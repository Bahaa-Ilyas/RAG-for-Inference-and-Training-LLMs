# Architecture: Decomposition RAG / sub-question RAG

## High-level architecture
Decomposition RAG / sub-question RAG uses Multiple focused retrieval calls planned from decomposed sub-questions. to address the specific failure mode: Single retrieval calls fail on multi-hop questions that require comparing entities, time periods, documents, or constraints. The system should expose evidence, scores, metadata, and citations rather than treating generation as a black box.

## Index-time pipeline
Ingest PDFs, webpages, tables, and other files; parse text and layout; split into chunks or structured elements; attach metadata; embed and index into sparse, dense, graph, table, or tool-aware stores as required. For this method, the important implementation point is: Generate sub-questions; retrieve evidence per sub-question; answer each with citations; synthesize; verify each final claim against sub-evidence.
