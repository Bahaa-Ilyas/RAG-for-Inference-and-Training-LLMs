# Building Decomposition RAG / sub-question RAG From Scratch
...
Source: [Least-to-Most Prompting Enables Complex Reasoning in Large Language Models](https://arxiv.org/abs/2205.10625). This local SVG is a redraw/synthesis of the important method architecture from the canonical paper or source, not a copied paper figure.
...
# Decomposition RAG / sub-question RAG

        ## Summary
        Decomposition RAG breaks a complex question into sub-questions, retrieves evidence for each, and synthesizes the final answer from the sub-results.

        ## Problem it solves
        Single retrieval calls fail on multi-hop questions that require comparing entities, time periods, documents, or constraints.

        ## When to use this method
        Use it for analytical questions, due diligence, scientific synthesis, legal comparison, and any task with explicit subparts.

        ## When not to use this method
        Avoid it when questions are simple, when decomposition introduces unsupported assumptions, or when the user needs a direct answer from one source.

        ## Core components
        Question decomposer, sub-query retrievers, evidence joiner, intermediate answer store, synthesizer, verifier.

        ## Input and output
        Input is a complex query. Output is a set of answered sub-questions plus a final answer with citations per claim.

        ## PDF use
        For PDFs, decomposition can search separate sections, appendices, and tables, then align sub-answers with page citations. For text PDFs, preserve page numbers, section headings, captions, reading order, and chunk offsets. For scanned PDFs, run OCR before chunking and store OCR confidence, page image references, and detected tables. Use Unstructured, Docling, Marker, PyMuPDF, pdfplumber, Apache Tika, or Tesseract depending on document quality. Long scientific papers usually need section-aware chunking, parent-child links, equation/caption retention, and table extraction.

        ## Webpage use
        For webpages, decomposition should keep domain, version, and freshness constraints attached to each sub-query. For webpages, crawl with robots.txt awareness, sitemap support, canonical URL normalization, crawl timestamps, content hashing, boilerplate removal, and duplicate detection. Use Trafilatura, BeautifulSoup, Scrapy, Playwright, readability extraction, and URL canonicalization. Treat retrieved web text as untrusted because prompt injection can be embedded in pages.

        ## Production stack
        LangGraph, LlamaIndex SubQuestionQueryEngine, DSPy, LangChain chains, hybrid retrievers, rerankers.

        ## Minimal example
        Break 'Compare warranty exclusions in policy A and B' into one retrieval for each policy and a synthesis step over cited exclusions.

        ## Pros
        - Strong for multi-hop questions
- Improves explainability
- Enables per-claim evidence tracking

        ## Cons
        - More LLM and retrieval calls
- Bad decompositions mislead retrieval
- Needs aggregation and contradiction handling
...
```bash
python decomposition_rag.py --explain
python examples/run_example.py
```
