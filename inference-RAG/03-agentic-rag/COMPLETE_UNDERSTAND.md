# Complete Understanding: Agentic RAG / multi-step retrieval

## Core idea
Agentic RAG lets an LLM plan, call retrievers or tools repeatedly, inspect intermediate evidence, and decide when enough grounding exists. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Single-pass RAG fails when a question requires multiple sources, conditional search, follow-up retrieval, or dynamic tools. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Iterative retrieval and tool use selected by a planner, often over hybrid, graph, web, and API retrievers..
...
## Mathematical view
The controller chooses action a_t from policy pi(a_t | q, evidence_1:t). Utility balances expected answer gain against retrieval/tool cost. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Agentic RAG / multi-step retrieval changes that by using Iterative retrieval and tool use selected by a planner, often over hybrid, graph, web, and API retrievers. and by explicitly addressing: Single-pass RAG fails when a question requires multiple sources, conditional search, follow-up retrieval, or dynamic tools.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Plan subgoals; select a retrieval or tool action; inspect evidence; revise the plan; stop when confidence and coverage thresholds are met; synthesize with citations.
