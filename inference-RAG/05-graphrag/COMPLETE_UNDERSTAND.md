# Complete Understanding: GraphRAG

## Core idea
GraphRAG builds an entity and relationship graph from a corpus and retrieves local neighborhoods or global community summaries for graph-grounded generation. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Flat chunk retrieval struggles with entity-heavy corpora, cross-document relationships, and questions that ask for global patterns rather than one passage. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Graph traversal plus chunk/vector retrieval; local search for entities and global search over community summaries..
...
## Mathematical view
A graph query can combine vector similarity sim(q,chunk), entity match score, path score product over edges, and community summary relevance. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. GraphRAG changes that by using Graph traversal plus chunk/vector retrieval; local search for entities and global search over community summaries. and by explicitly addressing: Flat chunk retrieval struggles with entity-heavy corpora, cross-document relationships, and questions that ask for global patterns rather than one passage.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Extract entities and relations; build graph; cluster communities; summarize communities; retrieve relevant entities and summaries; ground answer in source spans.
