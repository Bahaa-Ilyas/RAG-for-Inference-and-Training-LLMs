# Complete Understanding: Hybrid RAG: BM25 + dense vector retrieval

## Core idea
Hybrid RAG combines sparse lexical retrieval such as BM25 with dense vector retrieval, then fuses the result set before generation. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Pure vector search misses exact terms, identifiers, names, SKUs, statutes, and error strings; pure keyword search misses paraphrases and semantic intent. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Sparse BM25 plus dense ANN with reciprocal rank fusion or weighted score fusion..
...
## Mathematical view
A common score is score(d,q)=RRF(rank_sparse)+RRF(rank_dense), where RRF(r)=1/(k+r). Weighted fusion can use alpha*norm(BM25)+(1-alpha)*cosine(q,d). Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Hybrid RAG: BM25 + dense vector retrieval changes that by using Sparse BM25 plus dense ANN with reciprocal rank fusion or weighted score fusion. and by explicitly addressing: Pure vector search misses exact terms, identifiers, names, SKUs, statutes, and error strings; pure keyword search misses paraphrases and semantic intent.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Index each chunk into sparse and dense stores; retrieve top candidates from both; normalize or rank-fuse; deduplicate by document and span; optionally rerank; build context.
