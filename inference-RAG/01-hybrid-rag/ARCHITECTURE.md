# Architecture: Hybrid RAG: BM25 + dense vector retrieval

## End-to-end responsibility
Hybrid RAG architecture is an end-to-end responsibility, not a retrieval-only component. The system owner is accountable for document ingestion, chunking strategy, metadata design, sparse and dense indexing, retrieval evaluation, answer construction, citation quality, and response safety. A production pipeline is only healthy when the path from raw source to final answer is observable and testable.

## High-level architecture
Hybrid RAG: BM25 + dense vector retrieval uses Sparse BM25 plus dense ANN with reciprocal rank fusion or weighted score fusion. to address the specific failure mode: Pure vector search misses exact terms, identifiers, names, SKUs, statutes, and error strings; pure keyword search misses paraphrases and semantic intent. The system should expose evidence, scores, metadata, and citations rather than treating generation as a black box.

## Index-time pipeline
Ingest PDFs, webpages, tables, and other files; parse text and layout; split into chunks or structured elements; attach metadata; embed and index into sparse, dense, graph, table, or tool-aware stores as required. For this method, the important implementation point is: Index each chunk into sparse and dense stores; retrieve top candidates from both; normalize or rank-fuse; deduplicate by document and span; optionally rerank; build context.
...
## Mermaid source (visible in preview)
To ensure the architecture code is visible in the preview body, the raw Mermaid source is shown below.
