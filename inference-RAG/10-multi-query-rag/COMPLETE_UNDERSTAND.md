# Complete Understanding: Multi-query RAG

## Core idea
Multi-query RAG generates several diverse search queries for one user question, retrieves for each, and fuses results. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
One query embedding captures only one view of intent and may miss relevant passages that use different terminology. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Parallel retrieval over multiple generated query variants..
...
## Mathematical view
Candidate set D = union_i R(r_i(q)); final score can use RRF across query-specific ranks plus a diversity penalty. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Multi-query RAG changes that by using Parallel retrieval over multiple generated query variants. and by explicitly addressing: One query embedding captures only one view of intent and may miss relevant passages that use different terminology.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Generate N variants; run retrievers in parallel; union results; deduplicate by source span; fuse ranks; rerank and answer.
