# Complete Understanding: Self-RAG / adaptive retrieval

## Core idea
Self-RAG trains or prompts a model to decide when to retrieve, evaluate retrieved passages, and critique its own generations. The method should be understood as an evidence-control strategy: it changes what information is eligible to reach the model, how that information is ranked or transformed, and how the final answer is checked.

## Intuition
Always retrieving can add noise, while never retrieving causes unsupported answers; standard RAG rarely knows when evidence is insufficient. The intuition is to add the missing control surface rather than asking the language model to compensate after retrieval has already failed. Better evidence selection usually improves faithfulness more reliably than a larger generator alone.
...
4. At query or training time, select evidence with Conditional retrieval directed by model self-reflection signals..
...
## Mathematical view
The model estimates probabilities for retrieve, relevance, support, and utility tokens; generation is conditioned on retrieved passages and reflection decisions. Evidence quality should be measured separately from answer quality because high answer scores can hide weak retrieval.

## Difference from standard RAG
Standard RAG retrieves a few chunks and generates. Self-RAG / adaptive retrieval changes that by using Conditional retrieval directed by model self-reflection signals. and by explicitly addressing: Always retrieving can add noise, while never retrieving causes unsupported answers; standard RAG rarely knows when evidence is insufficient.
...
Use deterministic document IDs, stable chunk IDs, source offsets, page and URL metadata, embeddings, sparse indexes when useful, rerankers when precision matters, and prompts that force citation to provided evidence. For this method specifically: Predict whether retrieval is needed; retrieve passages; score relevance and support; generate; critique output; revise or abstain.
